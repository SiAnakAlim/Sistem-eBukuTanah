...
$query = "SELECT * FROM peminjaman_warkah WHERE id = $id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_POST['tanda_tangan'])) {
        ...
        $tanda_tangan = "uploads_warkah/ttd_" . time() . ".png";
        file_put_contents($tanda_tangan, $data);
    }

    // Query update data
    $update_query = "UPDATE peminjaman_warkah SET 
    ...
    if ($conn->query($update_query) === TRUE) {
        echo "<script>alert('Data berhasil diperbarui!'); window.location.href='tabel_warkah.php';</script>";
    } else {
...
<div class="container mt-4">
    <h1>Edit Data Peminjaman Warkah</h1>
    <a href="tabel_warkah.php" class="btn btn-secondary mb-3">Kembali</a>

    <form action="update_warkah.php?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
        <div class="mb-3">
        ...
        <div class="mb-3">
            <label for="jenis_hak" class="form-label">Nomor 208</label>
        ...
        <div class="mb-3">
            ...
            <?php if (!empty($row['file_keterangan'])): ?>
                <small>File sebelumnya: <a class="file-link" href="uploads_warkah/<?php echo $row['file_keterangan']; ?>" target="_blank">Lihat File</a></small>
            <?php endif; ?>
        </div>
        <div class="mb-3">
            ...
            <?php if (!empty($row['tanda_tangan'])): ?>
                <small>File tanda tangan sebelumnya: <a class="file-link" href="uploads_warkah/<?php echo $row['tanda_tangan']; ?>" target="_blank">Lihat File</a></small>
            <?php endif; ?>
        </div>
        <!-- Form fields here -->
