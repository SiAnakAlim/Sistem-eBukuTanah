...
// Query utama
$query = "SELECT * FROM peminjaman_warkah $where_sql ORDER BY tanggal_pinjam ASC, id ASC LIMIT $limit OFFSET $offset";
$result = $conn->query($query);

// Total data (dengan filter yang sesuai)
$total_query = "SELECT COUNT(*) as total FROM peminjaman_warkah $where_sql";
$total_result = $conn->query($total_query);
$total_data = $total_result->fetch_assoc()['total'] ?? 0;
$total_pages = ceil($total_data / $limit);

// Statistik total
$dikembalikan_result = $conn->query("SELECT COUNT(*) as dikembalikan FROM peminjaman_warkah WHERE status = 'Dikembalikan'")->fetch_assoc();
$dipinjam_result = $conn->query("SELECT COUNT(*) as dipinjam FROM peminjaman_warkah WHERE status = 'Dipinjam'")->fetch_assoc();
?>
...
<div class="container mt-4">
    <h1>Data Peminjaman Warkah</h1>

    <div class="top-actions mb-3">
        <a href="index.php" class="btn btn-secondary">Kembali</a>
        <a href="form_warkah.php" class="btn btn-success">+ Tambah Input Baru</a>
        <form action="laporan_warkah.php" method="GET" class="mb-3 d-flex justify-content-end">
        ...
        <table class="table table-striped table-bordered text-center" id="peminjamanTable">
            <thead>
                <tr>
                    ...
                    <th class="jenis-hak-col">Nomor 208</th>
                    <th>Status</th>
        ...
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr id='row_{$row['id']}' class='animated-fade'>
                    ...
                    <td>" . (!empty($row['file_keterangan']) ? "<a href='uploads_warkah/" . htmlspecialchars($row['file_keterangan']) . "' target='_blank'>Lihat</a>" : "-") . "</td>
                    <td>
                        <a href='edit_peminjaman_warkah.php?id=" . $row['id'] . "' class='btn btn-warning btn-sm btn-action'>Edit</a>";
...
<script>
    ...
    if (confirm("Apakah Anda yakin ingin menghapus data ini?")) {
        window.location.href = 'hapus_peminjaman_warkah.php?id=' + id;
    }
}
