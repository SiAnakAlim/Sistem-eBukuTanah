...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Ambil data lama
    $sql_get = "SELECT file_keterangan FROM peminjaman_warkah WHERE id = ?";
    ...
    if (!empty($_FILES["file_keterangan"]["name"])) {
        ...
        if (!empty($old_file) && file_exists("uploads_warkah/" . $old_file)) {
    ...
    $sql_update = "UPDATE peminjaman_warkah SET nama_peminjam=?, tanggal_pinjam=?, jenis_hak=?, status=?, keterangan=?, file_keterangan=?, tanggal_kembali=$tanggal_kembali WHERE id=?";
